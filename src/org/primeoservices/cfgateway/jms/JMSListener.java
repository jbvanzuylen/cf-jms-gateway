...
public class JMSListener extends JMSExchanger implements MessageListener
{
	...
	/**
	private void onMessageFailure(final Message message)
	{
		this.failureCount++;
		if (this.failureCount > maxFailure && maxFailure > -1)
		{
	...
	private void ackMessage(final Message message)
	{
		message.acknowledge();
		this.failureCount = 0;
		if (this.session.getTransacted()) this.session.commit();
